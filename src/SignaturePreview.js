...
const buildPhoneMarkup = ({ number, type, note }, index) => {
  ...
  let body = `${!!type ? `${type} ` : ""}${number}${!!note ? ` ${note}` : ""}`
  if (index === 0) {
...
const SignaturePreview = ({ name, titles, phones, extraContacts = [] }) => {
  ...
  htmlPreview = htmlPreview.replace('$NAME$', name);
  htmlPreview = htmlPreview.replace('$TITLES$', titles.map(buildTitleMarkup).join(''));
  htmlPreview = htmlPreview.replace('$PHONES$', phones.map(buildPhoneMarkup).join(''));
  htmlPreview = htmlPreview.replace('$EXTRA_CONTACTS$', extraContacts.map(buildExtraContact).join(''));
